...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        self.v_universe = np.arange(-12, 12.1, 0.1)
        
        self.v_mf = {
            "fast_left":  fuzz.trimf(self.v_universe, [-12, -12, -6]),
            "slow_left":  fuzz.trimf(self.v_universe, [-8, -4, 0]),
            "stop":       fuzz.trimf(self.v_universe, [-1, 0, 1]),
            "slow_right": fuzz.trimf(self.v_universe, [0, 4, 8]),
            "fast_right": fuzz.trimf(self.v_universe, [6, 12, 12])
        }
        ...
        self.rules = {
            ...
            "center":    "stop",
            "right":     "slow_right",
            "far_right": "fast_right"
        }
        
    def act(self, x_diff: int, y_diff: int):
        velocity = self.make_decision(x_diff)
        self.move(self.racket.rect.x + velocity)
    
    def make_decision(self, x_diff: float):
        x_memberships = {name: fuzz.interp_membership(self.x_universe, mf, -x_diff)
                         for name, mf in self.x_mf.items()}
        
        aggregated = np.zeros_like(self.v_universe)
        for x_name, mu in x_memberships.items():
            if mu > 0:
                v_name = self.rules[x_name]
                clipped_mf = np.fmin(mu, self.v_mf[v_name])
                aggregated = np.fmax(aggregated, clipped_mf)
        
        vel = fuzz.defuzz(self.v_universe, aggregated, 'centroid')
        
        scale = 1 + abs(self.ball.x_speed) / 5
        vel *= scale
